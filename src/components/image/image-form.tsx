...
import { FolderOpen, Image as ImageIcon, Link2, Loader2, Plus, Save, Trash2, Upload } from "lucide-react";
...
interface ImageFormProps {
  ...
  contentId?: string;
  onSubmit: (data: ImageSubmitData) => Promise<void>;
  isSubmitting: boolean;
}

export function ImageForm({ mode, initialData, contentId, onSubmit, isSubmitting }: ImageFormProps) {
  const siteConfig = useSiteConfig();
  const uploadEnabled = siteConfig?.fileUploadEnabled ?? false;

  const [selectedTags, setSelectedTags] = useState<TagItem[]>([]);
  const [newTags, setNewTags] = useState<string[]>([]);
  const [imageUrls, setImageUrls] = useState<string[]>(mode === "edit" ? [""] : []);
  ...
  const handleSubmit = async (data: ImageFormData) => {
    if (validImages.length === 0) {
      toast.error(mode === "create" ? "请至少添加一张图片" : "请至少保留一张图片链接");
      return;
    }
            ...
            <Card>
              <CardHeader className="pb-4">
                ...
                <CardDescription className="mt-1">
                  {mode === "create" ? "上传图片或添加外链，至少一张" : "管理图片 URL 链接，至少保留一张"}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {mode === "create" ? (
                  /* 创建模式：Tabs（上传 / 外链 / 我的文件） */
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-muted-foreground">已添加 {validImages.length} 张有效图片</p>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
                ...
                {validImages.length > 0 && (
                  <div className="space-y-3">
                    ...
                    <div
                      className={cn(
                        "grid gap-2",
                        mode === "create" ? "grid-cols-3 sm:grid-cols-4 md:grid-cols-5" : "grid-cols-4",
                      )}
                    >
                      {validImages.map((url, i) => (
                        <div
                          ...
                          className={cn(
                            ...
                            mode === "create" ? "aspect-square" : "aspect-square rounded-md",
                          )}
                        >
