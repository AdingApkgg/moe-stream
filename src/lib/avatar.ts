 ...
 */
export function getAvatarUrl(email: string | null | undefined, size = 80): string | undefined {
  if (!email) return undefined;
  
  const normalizedEmail = email.toLowerCase().trim();
  
  // 检查是否是 QQ 邮箱
  const qqMatch = normalizedEmail.match(/^(\d{5,11})@qq\.com$/);
  if (qqMatch) {
    const qqNumber = qqMatch[1];
    return `https://q1.qlogo.cn/g?b=qq&nk=${qqNumber}&s=100`;
  }
  
  // 使用 WeAvatar（兼容 Gravatar 的国内服务）
  const hash = md5(normalizedEmail);
  return `https://weavatar.com/avatar/${hash}?s=${size}&d=identicon`;
}

// 客户端和服务端使用同一个函数
export const getAvatarUrlClient = getAvatarUrl;
