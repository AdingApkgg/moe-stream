...
import {
  ChevronDown, Download, FileVideo, Gamepad2, Image as ImageIcon,
...
export function GameSingleUpload() {
  ...
  const extraFilledCount = useMemo(() => {
    let count = 0;
    if (form.getValues("originalName") || form.getValues("originalAuthor")) count++;
    ...
    return count;
  }, [form, screenshots, videos, downloads, versions, customTabs]);
                  ...
                  <CardContent className="pt-0">
                    <Tabs defaultValue="origin" className="w-full">
                      ...
                      <TabsContent value="screenshots" className="space-y-4 mt-4">
                          ...
                          {screenshots.map((url, i) => (
                            <div key={i} className="flex gap-2 items-start">
                              <div className="flex-1">
                                <UrlOrUploadInput value={url} onChange={(v) => { const s = [...screenshots]; s[i] = v; setScreenshots(s); }} accept="image/*" placeholder="https://example.com/screenshot.jpg" contentType="game" />
                              </div>
                        ...
                        <p className="text-xs text-muted-foreground">已添加 {screenshots.filter(s => s.trim()).length} 张截图</p>
                      ...
                      <TabsContent value="videos" className="space-y-4 mt-4">
                        ...
                        <div className="space-y-2">
                          {videos.map((url, i) => (
                            <div key={i} className="flex gap-2 items-start">
                              <div className="flex-1">
                                <UrlOrUploadInput value={url} onChange={(v) => { const vs = [...videos]; vs[i] = v; setVideos(vs); }} accept="video/*,.m3u8" placeholder="https://example.com/preview.mp4" contentType="game" />
                              </div>
                        ...
                        <p className="text-xs text-muted-foreground">已添加 {videos.filter(s => s.trim()).length} 个视频</p>
                      </TabsContent>
                        ...
                        {versions.map((ver, i) => (
                          <div key={i} className="p-3 border rounded-lg bg-muted/30 space-y-3">
                            <div className="flex items-center gap-2">
                              ...
                              <Input placeholder="版本标记，如：v1.0、v2.0 汉化版" value={ver.label} onChange={(e) => { const u = [...versions]; u[i] = { ...u[i], label: e.target.value }; setVersions(u); }} className="flex-1" />
                              <Button type="button" variant="ghost" size="icon" onClick={() => setVersions(versions.filter((_, j) => j !== i))}><Trash2 className="h-4 w-4" /></Button>
                            </div>
                            <MdxEditor value={ver.description} onChange={(val) => { const u = [...versions]; u[i] = { ...u[i], description: val }; setVersions(u); }} placeholder="版本描述（更新内容、注意事项等），支持 Markdown..." maxLength={10000} minHeight="100px" />
                          </div>
                        ))}
                      ...
                      <TabsContent value="customTabs" className="space-y-4 mt-4">
                        ...
                        {customTabs.map((tab, i) => (
                          <div key={i} className="p-3 border rounded-lg bg-muted/30 space-y-3">
                            <div className="flex items-center gap-2">
                              <GripVertical className="h-4 w-4 text-muted-foreground shrink-0" />
                              <Select value={tab.icon || "file-text"} onValueChange={(val) => { const u = [...customTabs]; u[i] = { ...u[i], icon: val }; setCustomTabs(u); }}>
                              ...
                              <Input placeholder="页面标题，如：攻略、MOD 列表" value={tab.title} onChange={(e) => { const u = [...customTabs]; u[i] = { ...u[i], title: e.target.value }; setCustomTabs(u); }} className="flex-1" />
                              <Button type="button" variant="ghost" size="icon" onClick={() => setCustomTabs(customTabs.filter((_, j) => j !== i))}><Trash2 className="h-4 w-4" /></Button>
                            </div>
                            <MdxEditor value={tab.content} onChange={(val) => { const u = [...customTabs]; u[i] = { ...u[i], content: val }; setCustomTabs(u); }} placeholder="页面内容，支持 Markdown / MDX 语法..." maxLength={50000} minHeight="150px" />
                          </div>
                        ))}
